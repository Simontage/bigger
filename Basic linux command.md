...
**目录的复制** 源是以/为结尾的，cp默认情况下是不会复制命令的，要使用选项-r（-R）表示递归复制目录下的所有文件。和文件的复制一样，如果目标路径最后一个单元并不存在，那么就将源目录复制到目标目录并且重命名为目标路径最后一个单元的名字。如果是一个已经存在的目录，那么就复制到该目录下并保持目录名；如果是一个已经存在的文件，那么就会创建一个指向源目录的链接文件
